...
export default function BrowsePage() {
  ...
  const filtered = useMemo(() => {
    ...
    rows = [...rows].sort((a, b) => sort === 'oldest' ? Number(a.year || 0) - Number(b.year || 0) : sort === 'title' ? a.title.localeCompare(b.title) : new Date(b.created_at || 0).getTime() - new Date(a.created_at || 0).getTime());
    return rows;
      ...
      <section className="container py-10">
        <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-zinc-900/90 via-black to-red-950/30 p-6 md:p-8">
          ...
          <div className="mt-6 grid gap-3 lg:grid-cols-[1fr_220px_180px_180px]">
            ...
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-4 outline-none focus:border-red-500">
              <option value="all">هەموو جۆرەکان</option>
              {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <select value={year} onChange={(e) => setYear(e.target.value)} className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-4 outline-none focus:border-red-500">
              <option value="all">هەموو ساڵەکان</option>
              {years.map((y) => <option key={String(y)} value={String(y)}>{y}</option>)}
            </select>
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="rounded-2xl border border-white/10 bg-zinc-950 px-4 py-4 outline-none focus:border-red-500">
